...
consensus_gsea_ccl4_bar = gsea_res_ccl4 %>% 
  ...
  geom_text(aes(label = str_c("p = ", scientific(pval,3), sep = " "), 
                y=0.95*NES), color="white", position=position_dodge(width=0), 
            size=4.5) +
  my_theme(grid = "y")
...
#### Enrichment of consensus gene sets with combined treatment signature
```{r}
gsea_res_treatment = readRDS("output/zonation/consensus_and_ind_gsea_res.rds") %>%
  filter(contrast_reference == "treatment",
         source == "consensus")

consensus_gsea_treatment_bar = gsea_res_treatment %>% 
  mutate(zonation = str_to_title(zonation)) %>%
  ggplot(aes(x=time, y=NES, fill=zonation)) +
  ...
  labs(x = "Time in weeks", fill = "Zonation") +
  theme(legend.position = "top") +
  geom_text(aes(label = str_c("p =", scientific(pval,3), sep = " "), y=0.95*NES),
            color="white", position=position_dodge(width=0.9), size=4.5) +
  my_theme(grid = "y")

saveRDS(consensus_gsea_treatment_bar, "fig/zonation/consensus_pc_pp_gsea_treatment.rds")
```
...
ind_gsea_oil_bar = gsea_res_oil_ind %>% 
  ...
  labs(x = "Time in weeks", fill = "Zonation") +
  theme(legend.position = "top") +
  geom_text(aes(label = gtools::stars.pval(padj), y=0.9*NES), color="white", 
            position=position_dodge(width=0.9), size=5) +
  my_theme("y")
...
#### Enrichment of individual gene sets with combined treatment signature
```{r}
gsea_res_treatment_ind = readRDS("output/zonation/consensus_and_ind_gsea_res.rds")%>%
  filter(contrast_reference == "treatment",
         source != "consensus")

ind_gsea_treatment_bar = gsea_res_treatment_ind %>% 
  mutate(zonation = str_to_title(zonation),
         source = str_to_title(source)) %>%
  ggplot(aes(x=time, y=NES, fill=zonation)) +
  geom_col(position=position_dodge()) +
...
saveRDS(ind_gsea_treatment_bar, "fig/zonation/ind_pc_pp_gsea_treatment.rds")
```
