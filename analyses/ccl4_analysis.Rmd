...
saveRDS(list(explained = explained, coords = coords), file = "output/ccl4_experiment/pca.rds")
```
...
### Enrichment analysis
#### GO enrichment
```{r "celltype deconvolution with xcell"}
...
dorothea_scores = limma_result %>%
  mutate(dorothea_scores = data %>% map(function(data) {
    run_viper(data, dorothea_regulon_mouse_v1, value_name = "t", id_name = "contrast")
  })) %>%
