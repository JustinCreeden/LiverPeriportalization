...
saveRDS(braeuning, "output/zonation/individual_gene_sets/braeuning_gene_sets.rds")
```
...
saito_pericentral = bind_rows(saito_pericentral_male, saito_pericentral_common) %>%
  drop_na(gene) %>%
  mutate(gene = alias2SymbolTable(gene, species = "Mm")) %>%
  drop_na(gene) %>%
  semi_join(mgi_symbols) %>%
...
```{r}
consensus_zonation_gene_sets = readRDS("output/zonation/consensus_gene_sets.rds") %>%
